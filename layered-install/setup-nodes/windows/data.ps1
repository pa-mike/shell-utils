...
if ($wsl) 
{
    ...
    wsl -d $distro -u root -e bash ../setup-nodes/devbase/data.sh -p "/opt/anaconda3" -u $installUser
    $timestamp = Get-Date -Format g; Write-Host "[$timestamp] $scriptName - " -ForegroundColor Green -NoNewline; 
        Write-Host "Successfully completed wsl data setup"
}
...
if ($extensions) 
{
    ...
    code --install-extension ..\setup-nodes\devbase\vscode\datateam\pa-datateam-vs-extensions-0.0.1.vsix
    $timestamp = Get-Date -Format g; Write-Host "[$timestamp] $scriptName - " -ForegroundColor Green -NoNewline; 
        Write-Host "Installing extension pack to wsl remote vscode in $distro"
    wsl -d $distro code --install-extension ../setup-nodes/devbase/vscode/datateam/pa-datateam-vs-extensions-0.0.1.vsix
}
